...
growth_result <- data.frame(time = seq(from = 1,
                                       to = 100))
...
growth_result$Pdiscrete <- growth_result$time %>% 
  map_dbl(~discrete_logistic_pop(P0 = p0, r=r, T = .x))

# add a column for ODE result - two steps
tmp <- ode(y=P0,
           times = growth_result$time,
           dexppop,
           parms = c(r))
growth_result$Pode = tmp[,2]
# gather both model results using a key called "Ptype", value "P"
growth_result_nice <- growth_result %>% 
  pivot_longer(cols = -time,
               names_to = 'method',
               values_to = 'P')

# plot with model type as color
ggplot(growth_result_nice,
       aes(x = time, y = P,
           col = method))+
  geom_point()
```
