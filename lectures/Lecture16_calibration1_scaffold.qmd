...
msage_long <- msage %>% 
  pivot_longer(cols = -c('obs', 'wy', 'day',
                         'month', 'year', 'date'),
               names_to = 'run',
               values_to = 'str')
...
res <- msage %>% 
  select(-c(date, wy, obs, day, year, wy)) %>% 
  map_dbl(nse, o = msage$obs)

best_parameter <- which.max(res)

# basic calibration would be picking a single parameters set
ggplot(msage, aes(as.Date(date), 
                   msage[,best_parameter]))+
  geom_point()+
  geom_line(aes(x = as.Date(date),
              y =obs),
              linewidth =.5,
